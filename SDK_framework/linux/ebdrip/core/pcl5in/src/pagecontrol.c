  ...
  if (throw_page) {
    ...
    if (print_state->allow_macro_overlay) {
      ...
      if (success) {
        ...
        success = execute_macro_definition(pcl5_ctxt, macro_info->overlay_macro_numeric_id, TRUE) ;
...
Bool pcl5op_ampersand_l_J(PCL5Context *pcl5_ctxt, int32 explicit_sign, PCL5Numeric value)
{

mode_chapter = value.integer;
printf("\n Getting mode value for gpsFrameChapterSet()from PCL command, mode=%d\n",mode_chapter);
return TRUE;
...
Bool pcl5op_ampersand_l_H(PCL5Context *pcl5_ctxt, int32 explicit_sign, PCL5Numeric value)
{
...
#ifdef DEBUG_LOGS
	printf("Paper Source received from input file: %d\n",paper_source);
#endif
  ...
  paper_type_slip = paper_source;
  printf("\n Getting paper_type value for gpsFrameChapterSet() from PCL command, paper_type=%d\n",paper_type_slip);
  if (! set_paper_source(pcl5_ctxt, paper_source))
    return FALSE ;
