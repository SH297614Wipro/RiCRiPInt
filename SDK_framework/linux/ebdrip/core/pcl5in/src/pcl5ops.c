...
static struct PCL5RegisterOp pcl5_ops[] = {
  ...
  { NAME_AND_LENGTH("&lJ"), ECD | PC | PE | ME | MC | EG,	   PCL5_INTEGER, pcl5op_ampersand_l_J }, /* Page order */
...
static Bool pcl5_dispatch_characters(PCL5Context *pcl5_ctxt, int32 byte)
{
  ...
  while (keep_processing) {
	if(event_Exit_Paper_flag == TRUE)
        ...
        if ( is_first_byte_of_two_byte_char(pcl5_ctxt, wide_ch)) {
          ...
          if (eof_reached)
          	{
          	pcl5op_control_FF(pcl5_ctxt); keep_processing = FALSE;
			break ;
          	}
        }
  ...
  if(event_Exit_Paper_flag == TRUE)
  {
  	status = pcl5_end_text_run(pcl5_ctxt, TRUE);
	return status;
  }	
...
Bool pcl5_execops(PCL5Context *pcl5_ctxt)
{
  ...
  /* If we are not done we either have control codes, PCL commands or
     more text. */
  while (! suspend_pcl5_interpreter && status && event_Exit_Paper_flag == FALSE) {
...
if(event_Exit_Paper_flag == TRUE)
	{ 
	ch = "FF"; 
	status = pcl5_dispatch_characters(pcl5_ctxt, ch); 
	suspend_pcl5_interpreter = TRUE; 
                         ...
                         PCLXL_TO_PCL5_PASSTHROUGH_STATE_INFO* state_info)
{
  ...
  if (pass_through_type != PCLXL_SNIPPET_JOB_PASS_THROUGH) {
    pcl5_id_cache_load_permanent_patterns(pcl5_ctxt->resource_caches.user);
    pcl5_id_cache_load_permanent_macros(&pcl5_ctxt->resource_caches);
    pcl5_fontss_load(pcl5_ctxt);
  }
