...
/* FOR CMM */
uint8 pclxlColorDataread_(PCLXL_PARSER_CONTEXT parser_context)
{
    HQASSERT(parser_context != NULL, "parser_context is NULL") ;

    PCLXL_CONTEXT pclxl_context = parser_context->pclxl_context;
    PCLXL_COLOR_OBJECT Color_Object = pclxl_context->Color_Object;

    return Color_Object.nColorProfile;
}
...
#define SETCOLORTREATMENT_RASTER_OBJECTS (4)
  ...
  uint8 ncolorprofile;
   ...
   /* CMM */
	ncolorprofile = CAST_UNSIGNED_TO_UINT8(pclxl_attr_get_uint(match[SETCOLORTREATMENT_COLOR_TREATMENT].result));
 
   
   Color_Object.nColorProfile = ncolorprofile ;
