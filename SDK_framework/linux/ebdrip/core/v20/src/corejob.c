...
#ifdef DEFAULT_PDL
/* TEC interface used in display.c */
tec_api_20140912 * tec_api = NULL ;
#endif
...
void corejob_running(DL_STATE *page)
{
  ...
  mps_telemetry_label(0, jobStart);
#ifdef DEFAULT_PDL
    /* Find the TEC API. at the sart of job for DL checking*/
  if (SwFindRDR(RDR_CLASS_API, RDR_API_TEC, 20140912, &vp, NULL)
      != SW_RDR_SUCCESS || vp == 0)
    HQFAIL("TEC API not registered") ;
  tec_api = (tec_api_20140912 *)vp ;
#endif
}
