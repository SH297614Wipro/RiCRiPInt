...
#ifdef DEFAULT_PDL
/* TEC API for Fuser Control setting via skin*/
extern tec_api_20140912 * tec_api ;
static int current_tec_value;
#endif
...
#ifdef DEFAULT_PDL
  /* reset TEC value to below Level Two (0)  to force a setting for a new page */
  current_tec_value = -1;
#endif
...
#ifdef DEFAULT_PDL
static int32 check_TEC_Value(DL_STATE *page, LISTOBJECT *lobj, Bool patdl)
{
  uint32 *tec_criteria ;
  uint32 bbox ;
  int value = -1 ;
  dl_color_t pdlc ; 
  dlc_tint_t tint  = DLC_TINT_WHITE;
  uint8 reproType ;
  UNUSED_PARAM (DL_STATE *, page) ;
  UNUSED_PARAM (Bool, patdl) ;


    /* ignore check if the TEC value has reached NORMAL */
    if((current_tec_value < 2)  && (lobj->opcode != RENDER_erase)  && (lobj->opcode < RENDER_shfill_patch))
    {
      /* Bounding box */
      bbox = (lobj->bbox.x2 - lobj->bbox.x1) * (lobj->bbox.y2 - lobj->bbox.y1) ;
      /* if text reduce the bbox by the number of characters */
      /***********************************************************************
        this is an approximation to allow the supplied tests files to function
        as the bbox submitted covers the complete text string */
      if(lobj->opcode == RENDER_char)
        bbox = bbox/lobj->dldata.text->nchars ;
      /***********************************************************************/
      /* Initialthe table has 2 x 5 entries for now; first item has switch BBox level */
      if(bbox > *(*tec_api->tec_criteria + tec_api->tec_values))
        tec_criteria = *tec_api->tec_criteria + tec_api->tec_values ;
      else
        tec_criteria = *tec_api->tec_criteria ;
      #ifdef DEBUG_LOGS
      printf("\nFuser Temperature, bbox = %d\n", bbox);
      #endif
      /* Dither check - disposition */
      reproType = DISPOSITION_REPRO_TYPE(lobj->disposition);
      /* REPRO_TYPE_PICTURE = 0, REPRO_TYPE_TEXT = 1, REPRO_TYPE_VIGNETTE = 2,
                                    REPRO_TYPE_OTHER = 3, REPRO_TYPE_ILLEGAL = 255 */
      if((reproType == REPRO_TYPE_PICTURE) || (reproType == REPRO_TYPE_TEXT))
      {
        /* Check for image object */
        if(lobj->opcode == RENDER_image)
        {
          IMAGEOBJECT *image = lobj->dldata.image ;
          int bpp = im_expandbpp( image->ime) ; 
            /*check for 8 bit */
          if(bpp == 8)
          {
            current_tec_value = 2 ;
            tec_api->set_level(current_tec_value) ;
	    #ifdef DEBUG_LOGS
            printf("\nFuser Temperature is set to NORMAL for bpp = 8\n");
	    #endif
            return 1 ;
          }
          else if(bpp == 1 || bpp == 2)
          {
	    #ifdef DEBUG_LOGS
	    printf("\nFuser Temperature calculation for bpp = %d\n", bpp);
	    #endif
            tint = im_expandtec(image->ime) ;
          }
        }
        else
        {
          /* BW or Gray check */
          dlc_from_dl_weak(lobj->p_ncolor, &pdlc) ;
          /* returns  DLC_TINT_BLACK, DLC_TINT_WHITE, DLC_TINT_OTHER */
          ...
          if(tint == DLC_TINT_OTHER)
          {
            ...
            uint32 i, ncomps, nalloc;
            COLORVALUE *erase_cvs ;       /* Expanded erase color */
            COLORANTINDEX *erase_cis ;    /* Colorant indices */
            GUCR_RASTERSTYLE *hr ;        /* Raster style */

        
            hr = gsc_getTargetRS(gstateptr->colorInfo);
            gucr_colorantCount(hr, &nalloc) ;
            erase_cvs = (COLORVALUE *)mm_alloc(mm_pool_temp, nalloc * (sizeof(COLORVALUE) + sizeof(COLORANTINDEX)),
                                                                                          MM_ALLOC_CLASS_SEP_OMIT);
            if (erase_cvs == NULL)
              return 0;
            erase_cis = (COLORANTINDEX *)(erase_cvs + nalloc);

            gucr_colorantIndices(hr, erase_cis, &ncomps) ;
            ...
            /* We determine a -ve print if _all_ the erasepage comes out as
               ...
               or clear. */
            ires = dlc_first_colorant(&pdlc, &iter, &ci, &cv) ;
            ...
            if ( !dlc_get_indexed_colorant(&pdlc, COLORANTINDEX_ALL, &allcv) ) {
              allcv = cv ;
            } else {
              HQASSERT(allcv == cv,
            ...
            for ( i = 0 ;; ) {
              if ( ires == DLC_ITER_COLORANT ) {
                ...
                ires = dlc_next_colorant(&pdlc, &iter, &ci, &cv) ;
              } else if ( i < ncomps ) {
            ...
            mm_free(mm_pool_temp, erase_cvs, nalloc * (sizeof(COLORVALUE) + sizeof(COLORANTINDEX))) ;
          }
        }
      }
      /* now check the table for the correct TEC value to be set */
      if(reproType == REPRO_TYPE_PICTURE)
      {
        if(tint == DLC_TINT_OTHER)
          value = tec_criteria[1] ;
        else if(tint == DLC_TINT_BLACK)
          value = tec_criteria[2] ;
      }
      else if(reproType == REPRO_TYPE_TEXT)
      {
        if(tint == DLC_TINT_OTHER)
          value = tec_criteria[3] ;
        else if(tint == DLC_TINT_BLACK)
          value = tec_criteria[4] ;
      }
      if(value > current_tec_value)
      {
	#ifdef DEBUG_LOGS
        printf("\nFuser Temperature - Dither Type = %d, Color Value = %d\n", reproType, tint);
	#endif
        current_tec_value = value ;
        tec_api->set_level(current_tec_value) ;
      }
      else if(value == -1)
      {
	#ifdef DEBUG_LOGS
        printf("\nFuser Temperature is not calculated, setting it to NORAML\n");
	#endif
        current_tec_value = 2;
        tec_api->set_level(current_tec_value) ;
      }
...
#ifdef DEFAULT_PDL
  if (tec_api != NULL && tec_api->tec_criteria != NULL)
  {
    check_TEC_Value(page, lobj, patdl);
  }
#endif
