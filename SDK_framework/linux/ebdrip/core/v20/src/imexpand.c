...
#ifdef DEFAULT_PDL
/* -------------------------------------------------------------------------- */
/* returns  DLC_TINT_BLACK, DLC_TINT_WHITE or DLC_TINT_OTHER to TEC calculation */
int32 im_expandtec(const IM_EXPAND *ime)
{
  size_t i;
  int32 value = DLC_TINT_WHITE ;
  if(ime->obpp == 8)
  {
    /* check all LUT for non zero - black or other*/
    for(i = 0; i < ime->lutsize; i++)
    {
      /* check LUT for max value and set black*/
      if(ime->expluts[0][i] == 0xff)
        value = DLC_TINT_BLACK ;
      else if(ime->expluts[0][i] > 0)
      {
        /* if not max value sand set other and exit */
        value = DLC_TINT_OTHER ;
        break ;
      }
    }
  }
