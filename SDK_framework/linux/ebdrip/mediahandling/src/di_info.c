...
extern int GPS_Color_getProfile(gwmsg_client_t 	*client, void **addr, long	*num);
extern int GPS_GetModelInfo(char  dummy, char num,char	*key, char	*category, 	unsigned char value_len);
...
extern long GPS_Color_getShrd(void);
extern int GPS_Color_getID2(int hdpi, int vdpi, int bit, int draw, unsigned char prt,unsigned char paper);
extern int gpsColor_getRID( gwmsg_client_t* client, int modeID, gps_color_rid_t *rID, long *rID_size );
...
/*	#endif 	*/	/* USE_IMAQ */
	...
	while( idx ) {	/* ”Ô†‡‚ÉŒŸõ */
		...
		if( devinfo->flag & DI_SUPPORT_EXT_DEVINFO_PARAM ) 
		{
			...
			if( sscanf((char *)outbuf,"%d,%d,%d,%d,%d,%d" ,&x,&y,&bit,&paper,&prt_mode,&dit_mode) != 6 ) 
			{
				search = 0;
			} 
			else 
		...
		else 
		{
			...
			if( sscanf((char *)outbuf,"%d,%d,%d",&x,&y,&bit) != 3 ) 
			{
				search = 0;
			} 
			else 
...
/*	#endif 	*/	/* USE_IMAQ */
	...
	for( i=1; i<=idx; i++ ) {
		...
		if( devinfo->flag & DI_SUPPORT_EXT_DEVINFO_PARAM ) {
			/* VŽd—l‚Íƒpƒ‰ƒ[ƒ^‚ª‚U‚Â */
			if( sscanf((char *)outbuf,"%d,%d,%d,%d,%d,%d"
...
int get_maxdit( int *max_dit ) /* Should it be static??*/
{
	...
	if( (get_appinfo_di_GPS("max_dit" ,CATEGORY_DI ,sizeof(buf),buf) != SERCH_KEY_OK)
		|| (sscanf((char *)outbuf, "%d", max_dit) != 1) ) {
...
int get_maxplane( int *max_plane )
{
	...
	if( (get_appinfo_di_GPS("max_plane" ,CATEGORY_DI ,sizeof(buf),buf) != SERCH_KEY_OK)
		|| (sscanf((char *)outbuf, "%d", max_plane) != 1) ) {
		/* ƒGƒ‰[‚Ìê‡‚ÍƒfƒtƒHƒ‹ƒg’l */
		*max_plane = DEF_PLANE;
	}
...
void adjust_dgparam_GPS( dg_param_t *para ) /* This API is static in GPS, and in here?? */
{
	...
	while( 1 ) {
		...
		if( (get_appinfo_di_GPS(strbuf
					...
					,sizeof(buf),buf) != SERCH_KEY_OK)
			|| (sscanf((char *)outbuf, "%d,%d,%d,%d,%d,%d,%d,%d", &org_resox,&org_resoy,&org_bit,&org_dit, &new_resox,&new_resoy,&new_bit,&new_dit) != 8) ) 
		{
			break;
		}
			...
			(para->bit == org_bit) && (para->dit == org_dit) ) 
		{
			...
			if (fflag == 0)
			{
				printf("trans=%dx%dx%d-%d->%dx%dx%d-%d",para->resox, para->resoy, para->bit, para->dit,new_resox, new_resoy, new_bit, new_dit);
				fflag = 1;
			}
...
int get_gcr_di_GPS(di_devinfo_GPS_t *devinfo)
{
	...
	dg_param_t dgparam;
devinfo->gcr_info = (di_gcrinfo_t *)mmalloc(sizeof(di_gcrinfo_t) * devinfo->resonum);
	if (devinfo->gcr_info == 0)
	{
		return DI_GET_NG;
	}
	gcr_ptr = devinfo->gcr_info;

devinfo->gcrhgr_info = (di_gcrinfo_t *)mmalloc(sizeof(di_gcrinfo_t) * devinfo->resonum);
	
	if (devinfo->gcrhgr_info == 0)
	{
		return DI_GET_NG;
	}
...
int get_bgucr_param_di_GPS(di_devinfo_GPS_t *devinfo)
{
	...
	else 
	{
		if( (get_appinfo_di_GPS("bgucr" ,category_di, sizeof(buf), buf) != SERCH_KEY_OK)
			|| (sscanf((char *)outbuf, "%f,%f,%f,%f", &devinfo->bg_startp, &devinfo->bg_slant, 
...
int get_tlimit_di_GPS(di_devinfo_GPS_t *devinfo)
{
 ...
 if( devinfo->toner_limit == TRUE ) { /* �g�i?[?���� �� */
 int idx;
 char strbuf[32];
 ...
 /* �?�ʋK?��l�p�^?[��?��擾 */
 if( devinfo->flag & DI_SUPPORT_EXT_DEVINFO_PARAM ) { /* �g���d�l */
 int reso;
