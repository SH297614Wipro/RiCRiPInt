...
void PrintSysStart_GPS()
{
        ...
        printf("bsw5 = [%d]\n",bsw5);
		BitSWConfig.is_mixedrot_old = (bsw5 & (1 << 5)) ? TRUE : FALSE;
...
int di_getinfo_GPS_update(di_devinfo_GPS_t *devinfo, char *dither_gamma_file)
{
	int val;
...
Uint8 PrintSysCheckQualityMode (Sint32 *om_media, OMenum *om_qmode)
{
    di_devinfo_GPS_t *diDevInfo;
    ...
    diDevInfo = devinfo;
	xres = OMGetCurrentInt(XRES, OMSYSTEM);
	yres = OMGetCurrentInt(YRES, OMSYSTEM);
...
*/
	/* Temporarily replacing the above calls */
	media = *om_media;
	qmode = *om_qmode;
	/* Combination check of resolution and paper kind */
	agree = 0;
	if(!diDevInfo->reso_set)
	{
				return FALSE;
...
Boolean PrintSysGetPaperInfo(OMenum paper)
{
    ...
    int gps_notify =0;
 	if(paper == _enum_None)
 		return FALSE;
	result = GPS_GetPaperInfo(gps_client, paper, &gpsPaperInfo, gps_notify);
 ...
 */
int PrintSysReinitTrayInfo()
{
	int SIZEOF_TRAYBINTBL = 10;
	gps_trayinfo_t  *trayinfo ;
 ...
 */
int PrintSysReinitBinInfo()
{
	int SIZEOF_TRAYBINTBL = 10;
	int CMDTYPE_OUTBIN = 2;
	gps_bininfo_t  *bininfo;
