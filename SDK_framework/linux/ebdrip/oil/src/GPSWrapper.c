...
int GPS_FrameChapterSet( gwmsg_client_t* client, int frameid, unsigned char mode, unsigned char paper_type );
...
if(!g_pstTrayInfo)
{
...
#endif
  if(!GPS_Tray_notify_flag)
    notify_flag = GPS_NOTIFY_CHANGE_OFF;
  else
  	notify_flag = GPS_NOTIFY_CHANGE_ON;
    if( -1 == gpsGetTrayInfo(gps_client, sysinfo.num_tray, gpsTrayInfo, &trayNum, notify_flag) )
      return -1;
  ...
  for(nTrayIndex = 0; nTrayIndex < trayNum; nTrayIndex++)
  {
    ...
    g_pstTrayInfo[nTrayIndex].eMediaSource = gpsTrayInfo[nTrayIndex].id;
	g_pstTrayInfo[nTrayIndex].ePaperSize  = gpsTrayInfo[nTrayIndex].p_size;
    g_pstTrayInfo[nTrayIndex].eMediaType =  gpsTrayInfo[nTrayIndex].p_kind;
#if 0
	...
	*/
    ...
    g_pstTrayInfo[nTrayIndex].nPriority =-1;/*nTrayIndex; *//* assumption yet to conform. 	*/
...
#endif
  if(!GPS_Bin_notify_flag)
    notify_flag = GPS_NOTIFY_CHANGE_OFF;
  else
  	notify_flag = GPS_NOTIFY_CHANGE_ON;

  if( -1 == gpsGetBinInfo(gps_client, sysinfo.num_bin, gpsBinInfo, &binInfo, notify_flag) )
    return -1;
...
{
		printf("------------------------------------- \n");
		printf("Call gpsFrameChapterSet\n");
		return gpsFrameChapterSet( client, frameid, mode, paper_type);
}
...
/***********************************************************************************************************
*Function Name: GPS_SoutFlush()
*Attributes:
*hostbuf 	IN 		Pointer to Host Buffer Structure obtained by gpsSoutOpen() function.
*Description:  This function Flushes buffer
************************************************************************************************************/
int GPS_SoutFlush(gps_hostbuf_p hostbuf)
{
	printf("------------------------------------- \n");
	printf("Call gpsSoutFlush\n");
	return gpsSoutFlush(hostbuf);
}
/***********************************************************************************************************
*Function Name: GPS_SoutWrite()
*Attributes:
*hostbuf 	IN 		Pointer to Host Buffer Structure obtained by gpsSoutWrite() function.
*Description:  This function writes the content to buffer
************************************************************************************************************/
long GPS_SoutWrite(gps_hostbuf_p hostbuf,unsigned char *buf,long len,int flag)
{
	printf("------------------------------------- \n");
	printf("Call gpsSoutWrite\n");
	return gpsSoutWrite(hostbuf,buf,len,GPS_HOST_NONBLOCK);
}
