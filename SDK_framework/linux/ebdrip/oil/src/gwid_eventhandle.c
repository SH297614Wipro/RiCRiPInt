...
int Get_Data_From_GPS()
{
    ...
    GPSBitSWRetVal = GPS_GetBitSw(gps_client,BIT_SW_002); /* bit number randomly given as BIT_SW_004 */
...
{
	int Jobstatus, Devicestatus, prt_notify = 0, prt_res, result;

	gwfcall_gpsPlotCancel_t gpsPltCncl;
	gwfcall_gpsFrameCancel_t gpsFrmCncl;
	gwmsg_GpsEv_Req_Exit_t gpsReqExit;
	gwmsg_GpsEv_Req_Testprint_t testPrint;
	gwmsg_GpsEv_Interp_Pause_Req_t	pause;
	gwmsg_GpsEv_Interp_Resume_Req_t	resume;
	gwmsg_GpsEv_Req_Testprint_t	*msg;
	gps_prtstatus_p prtstatus;
    ...
    switch(m->type)
    {
		...
		case GWID_GpsEv_Req_Cancel_Data: 
							...
							(void)gpsInterpFinishProcData(&gps_client, processJobData->ujobid, GPS_INTERP_FIN_CANCEL, 0);
							(void)gpsInterpNotifyState(&gps_client, GPS_INTERP_STATE_FLUSHING);
							frameID = gpsFrmCncl.frameid; 
							(void)gpsFrameCancel(&gps_client, frameID);
							plotID = PlotSet_plotid; /*gpsPltCncl.plotid;*/
							(void)gpsPlotCancel(&gps_client, plotID);
                            break;
		case GWID_GpsEv_Notify_Offline: 
							...
							(void)gpsInterpNotifyOffline(&gps_client);							
							break;
		case GWID_GpsEv_Notify_Online: 
							...
							(void)setDeviceStatus(Devicestatus);
							(void)gpsInterpNotifyOnline(&gps_client);
							break;
