...
*/
char    *OILVersion = DEBUG_STR "4.0r5" OIL_BUILD_INFO;
...
int g_bPageChecksum;
/* Flag say Job is for printing Banner Page */
int IsBannerPageJob;
/* AppendJob = True - Following current job, there is one more job to process */
int AppendJob;
...
#endif
{
  ...
  *pbSubmittedJob = FALSE;
...
#endif
  {
    ...
    *pbSubmittedJob = TRUE;
	AppendJob = pms_ptJob->uPrintBannerPage == GPS_PENV_VAL_INT_BANNERPAGEPRINT_OFF ? FALSE: TRUE;
	IsBannerPageJob = FALSE; //Every time initialize to False.
	
	if(pms_ptJob->uPrintBannerPage == GPS_PENV_VAL_INT_BANNERPAGEPRINT_BEFOREJOB)
	{
      if(AuthenticateBannerPagePrint()) 
      {
        //Print Banner page at begining.
	    IsBannerPageJob = TRUE; //On going job is banner page print.
        retVal = OIL_MultiThreadedStart( pms_ptJob, OIL_PDL_PCL5c );
		IsBannerPageJob = FALSE; // End of Banner page print job.
      }
	  AppendJob = FALSE;
    }
    else if(pms_ptJob->uPrintBannerPage == GPS_PENV_VAL_INT_BANNERPAGEPRINT_AFTERJOB)
    {
      if(FALSE == AuthenticateBannerPagePrint())
	  	AppendJob = FALSE; //Don't Print Banner Page.
	}
	
    retVal = OIL_MultiThreadedStart( pms_ptJob, eOIL_PDL );
	
	if(AppendJob == TRUE) //Print Banner page at end
	{
	  //On going job is banner page print.
	  IsBannerPageJob = TRUE; AppendJob = FALSE;
      OIL_MultiThreadedStart( pms_ptJob, OIL_PDL_PCL5c );
	  IsBannerPageJob = FALSE;// End of Banner page print job.
    }
...
int  find_PDLType(int *pPDL)
{
	OIL_eTyPDLType ePDLType = OIL_PDL_Unknown;
        int32 fGotPDL = FALSE;
    printf("\nCurrent PDL id recieved from GPS = %d\n",current_pdlid);
