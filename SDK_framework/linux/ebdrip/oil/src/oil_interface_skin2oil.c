...
#define FLAG_FIRST_PAGE 295963
#define FLAG_EXCEPT_FIRST_PAGE 32770
#define FINISH 1
#define FLAG_PAGE_END 4

  

/* extern variables */
extern int GPSPenvOpenretval;
extern gps_pageparam_shm_t param;
...
extern int Mixedrot_type;
extern int Binding_type;
extern int FP_force;
extern int g_JobEnd,g_JobStart;
extern int ProcessAllPages,PrintUptoLastPage,StopPrinting,PageCount;
extern int JobEndFlag;
extern int longEdge_val, shortEdge_val;
static int Rot_Flag;
extern int cur_media_val;
extern int GPS_FrameCreateEx(gwmsg_client_t *client, int framecreate_plotid, const gps_pagemode2_t *pagemode2, const gps_pageinfo2_t *pageinfo2, int	*framecreate_result);
extern int GPS_FrameCreateInfoRes(gwmsg_client_t *client, int FCinfo_plotid, gps_pagemode_p pagemode, gps_pageinfo_t *pageinfo, int FCinfo_force, gwmsg_GpsPage_FrameCreate_Res_t	*pFCR);
extern int GPS_FrameGetBandRes(gwmsg_client_t *client, int	FGband_frameid, int FGband_bandid, int FGband_planeid, gwmsg_GpsPage_FrameGetBand_Res_t *pFGBR);
extern PMS_TyBandPacket *ptBandPacket;
extern int bc_in_createbandpacket;
extern int auto_tray_change;
extern int tray_search_mode;
typedef int bool;
...
{
  int32 nRetVal = 0;
  int nFrameNumber = pRasterDescription->separation - 1;
  ...
  if(g_tSystemInfo.bHWA == TRUE)
       OIL_HWA1_RasterDescription(pRasterDestination, pRasterDescription);
  return nRetVal;
}
...
void UpdatePlotSetPageParamShm()
{

    int GPSPlotSetPageParamShmretval;
    int GPSPlotChangeBinding;
    int PlotSet_result , i;
	...
	gps_var = GPS_PENV_VAR_ID_OUTBIN;
        ...
        if(GPSPenvGetDefListRetVal >= 0)
		{
			...
			else
            {
				...
				g_pstCurrentJob->tCurrentJobMedia.uOutputTray = penv_val;
				param.output_tray= penv_val;
...
/*		if( Mixedrot_type)
		param.mixeddirrot = Mixedrot_type; */
		param.mixeddirrot = 1;/* hard coding for testing the MIX_Rotate feature SHOULD BE REMOVED */
		if( Binding_type )
		param.binding= Binding_type;
		
#ifdef DEBUG_LOGS
	printf(" Parameters in GPS_PlotSetPageParamShm are \n");
	for(i=0;i<GPS_MAX_PAGEPARAM_SHM;i++)
		printf("param.flag[%d] = %lu ",i,param.flag[i]);

		printf("\nparam.finish = %u ,param.limitless_exit = %u,param.output_tray = %u,param.qty= %u param.mixeddirrot = %u param.binding = %u \n",param.finish,param.limitless_exit,param.output_tray,param.qty,param.mixeddirrot,param.binding);

#endif
GPSPlotChangeBinding= gpsPlotChangeBinding(gps_client, PlotSet_plotid, param.binding , &PlotSet_result);
#ifdef DEBUG_LOGS
	printf("gpsPlotChangeBinding : %d\n", PlotSet_result);
#endif
                                 ...
                                 unsigned char * pBuffer )
{
  ...
  static int j = 0;
   int x, a;
  unsigned char Cur_media_type;
  di_chkduplexok_t dichkduplex;
  int Duplex_ok,CanDuplexFlag; 
#ifdef USE_64
  ras64 *pDestPtr64;
#else
#endif
...
/* GPS expect data in little Indian */
#ifdef LOWBYTEFIRST
  ...
  while (1)
  {
  	if(event_Cancel_flag == TRUE )break;
	if(event_Exit_Paper_flag == TRUE)break;
	if(JobEndFlag == 1 )break;
	...
	Cur_media_type = (unsigned char)cur_media_val;
	dichkduplex.pkind=cur_media_val;
	dichkduplex.psize=g_pstCurrentJob->tCurrentJobMedia.ePaperSize;
	gpsChkDuplexOk(gps_client , &dichkduplex, &Duplex_ok);
	CanDuplexFlag = (Duplex_ok==GPS_DUPLEX_CHK_OK);
#endif
	...
	if (g_pstCurrentPage->uPageNo == 1)
	{

		param.flag[0] = FLAG_FIRST_PAGE;
		param.finish = FINISH;
	}
	else

	{
		...
		param.flag[0] = FLAG_EXCEPT_FIRST_PAGE;
			
	}
	if((auto_tray_change == DI_ENV_VAL_INT_AUTOTRAYCHANGE_ON) || (tray_search_mode == DI_ENV_VAL_INT_TRAYSEARCHMODE_SWITCH))
	{
		param.limitless_exit =1;
	}
	else
	{
		param.limitless_exit  =0;
	}


	UpdatePlotSetPageParamShm();
...
//  pagemode.frame_length= nLinesInFrame;//nFramesToWrite; //4760;
  ...
  //pagemode.face = 1;
  /********Updating Face and Print_face values in the pagemode and pageinfo2 structure********/
   if(!param.mixeddirrot)
  {
     if(g_pstCurrentPage->bDuplex)
     {
       if(g_pstCurrentPage->uPageNo%2)
       {
	     pageinfo2.print_face = GPS_FACE_DPX1ST;
	     pagemode.face =  GPS_FACE_DPX1ST;
       }
	   else
	   {
	     pageinfo2.print_face = GPS_FACE_DPX2ND;
	     pagemode.face = GPS_FACE_DPX2ND;
	   }
     }
     else
     {
       pageinfo2.print_face = GPS_FACE_SINGLE;
	   pagemode.face = GPS_FACE_SINGLE;
     }
  }
 /********Updating rotate value in the pagemode structure********/
  if(!param.mixeddirrot)
  {
    if(longEdge_val)
    {
      if(g_pstCurrentPage->uOrientation == GPS_PENV_VAL_INT_ORIENTATION_PORTRAIT || g_pstCurrentPage->uOrientation == GPS_PENV_VAL_INT_ORIENTATION_REVERSEPORTRAIT)
    	pagemode.rotate = GPS_ROT_180;  
      if(g_pstCurrentPage->uOrientation == GPS_PENV_VAL_INT_ORIENTATION_LANDSCAPE || g_pstCurrentPage->uOrientation == GPS_PENV_VAL_INT_ORIENTATION_REVERSELANDSCAPE)
		pagemode.rotate = GPS_ROT_90;
    }
    else if(shortEdge_val)
    {
      if(g_pstCurrentPage->uOrientation == GPS_PENV_VAL_INT_ORIENTATION_PORTRAIT || g_pstCurrentPage->uOrientation == GPS_PENV_VAL_INT_ORIENTATION_REVERSEPORTRAIT)
	    pagemode.rotate = GPS_ROT_90;
      if(g_pstCurrentPage->uOrientation == GPS_PENV_VAL_INT_ORIENTATION_LANDSCAPE || g_pstCurrentPage->uOrientation == GPS_PENV_VAL_INT_ORIENTATION_REVERSELANDSCAPE)
	    pagemode.rotate = GPS_ROT_0;
    }
  }
  else
  {
	...
	else if(Rot_Flag)
	{
	  ...
	  {
	  	if(g_pstCurrentPage->uOrientation == GPS_PENV_VAL_INT_ORIENTATION_PORTRAIT || g_pstCurrentPage->uOrientation == GPS_PENV_VAL_INT_ORIENTATION_REVERSEPORTRAIT)
  	      pagemode.rotate = GPS_ROT_270;
	  }
  	}
  }
  /********Updating Binding value in the param structure********/
  if(g_pstCurrentPage->uOrientation == GPS_PENV_VAL_INT_ORIENTATION_PORTRAIT || g_pstCurrentPage->uOrientation == GPS_PENV_VAL_INT_ORIENTATION_REVERSEPORTRAIT)
  {
	if(g_pstCurrentJob->uBindingType==GPS_PENV_VAL_INT_BINDING_LONGEDGE)
      param.binding = GPS_BIND_LEFT;  
  	if(g_pstCurrentJob->uBindingType==GPS_PENV_VAL_INT_BINDING_SHORTEDGE )
	  param.binding = GPS_BIND_TOP;  
  }
  else if(g_pstCurrentPage->uOrientation == GPS_PENV_VAL_INT_ORIENTATION_LANDSCAPE || g_pstCurrentPage->uOrientation == GPS_PENV_VAL_INT_ORIENTATION_REVERSELANDSCAPE)
  {
	if(g_pstCurrentJob->uBindingType==GPS_PENV_VAL_INT_BINDING_LONGEDGE)
      param.binding = GPS_BIND_TOP;  
  	if(g_pstCurrentJob->uBindingType==GPS_PENV_VAL_INT_BINDING_SHORTEDGE )
...
#ifdef DEBUG_LOGS	
      printf("\n LIMITLESS_FEED ENABLED \n");
#endif	  
  }
  else
  {
      pageinfo.limitless_feed =0;
#ifdef DEBUG_LOGS		  
      printf("\n LIMITLESS_FEED DISABLED \n");
#endif	  
...
//need to check
  ...
  pageinfo.limitless_feed =1;
  pageinfo.paper_count=g_pstCurrentJob->uPagesToPrint   ;//1;
  if(g_pstCurrentJob->bSuppressBlank == TRUE)/*Counter specifictaion*/
  	{
  		if(g_pstCurrentJob->bDuplex == TRUE)
  			{
  				pageinfo.count_off = GPS_COUNT_OFF;
  				if(param.Cover.front != GPS_PENV_VAL_INT_FRONTCOVERPRINT_OFF)
  				{
					pageinfo.count_off = GPS_COUNT_ON;	
#ifdef DEBUG_LOGS
					printf("Duplex Job: Counting for blank page:%d\n", pageinfo.count_off);
#endif
  				}
				if(param.Cover.back != GPS_PENV_VAL_INT_BACKCOVERPRINT_OFF)
					pageinfo.count_off = GPS_COUNT_OFF;
#ifdef DEBUG_LOGS
					printf("Duplex Job: Counting for blank page:%d\n", pageinfo.count_off);
#endif				
		...
		else /*simplex job*/
			{
					pageinfo.count_off = GPS_COUNT_OFF;
#ifdef DEBUG_LOGS
					printf("Simplex Job: Counting for blank page:%d\n", pageinfo.count_off);
#endif				
			}
 ...
 /*****************************Call to gpsFrameCreateInfoRes*****************************/
	...
	else
	{
		...
		GetBandInfoFromPMS(nOutputPixelsPerLineIncRIPPadding, g_pstCurrentPage->nPageHeightPixels,nOutputDepth, &stPMSBandInfo);
          ...
          for ( k = 0 ; k < nColorsInThisBand ; k++ )
          {
            ...
            if(bPartialBand == TRUE)
            {
              ...
              if (g_ConfigurableFeatures.eBandDeliveryType == OIL_PUSH_BAND)
              {
                ...
                pDestPtr = ptCurrentBandPacket->atColoredBand[i].pBandRaster + (nLinesAlreadyWritten * nOutputBytesPerLine);
              }
              else
              {
  ...
  /*                unsigned char uPartialByte;
            ...
            if(bPartialBand == TRUE)
            {
              ...
              if (g_ConfigurableFeatures.eBandDeliveryType == OIL_PUSH_BAND ||
                  ...
                  g_ConfigurableFeatures.eBandDeliveryType == OIL_PUSH_BAND_DIRECT_FRAME)
              {
                ptCurrentBandPacket->atColoredBand[i].cbBandSize = (nLinesThisTime + nLinesAlreadyWritten) * nOutputBytesPerLine;
                ptCurrentBandPacket->atColoredBand[i].uBandHeight = nLinesThisTime + nLinesAlreadyWritten;
              }
              else
              {
            ...
            else
            {
              ...
              if (g_ConfigurableFeatures.eBandDeliveryType == OIL_PUSH_BAND ||
                  ...
                  g_ConfigurableFeatures.eBandDeliveryType == OIL_PUSH_BAND_DIRECT_FRAME)
              {
                ptCurrentBandPacket->atColoredBand[i].cbBandSize = nLinesThisTime * nOutputBytesPerLine;
                ptCurrentBandPacket->atColoredBand[i].uBandHeight = nLinesThisTime;
              }
              else
	 ...
	 int PlaneID_flush = GPS_COLOR_K;
	 for(j=0; j < OIL_MAX_PLANES_COUNT; j++)
        { 
            switch(Map[j])
            {
                case OIL_Cyan:
                    PlaneID_flush = GPS_COLOR_C;
                    break;
                case OIL_Magenta:
                    PlaneID_flush = GPS_COLOR_M;
                    break;
                case OIL_Yellow:
                    PlaneID_flush = GPS_COLOR_Y;
                    break;
                case OIL_Black:
                    PlaneID_flush = GPS_COLOR_K;
                    break;
                case OIL_InvalidColor:
                default:
                PlaneID_flush = -1;
		...
		GPSFrameFlushBandResRetVal =GPS_FrameFlushBandRes(gps_client,1,iBandNumber,PlaneID_flush,GPS_BAND_DRAW,GPS_FLUSH_SOON,&pFFBR);	
    ...
    default:
      ...
      return TRUE;
		...
		if(event_Cancel_flag == TRUE|| JobEndFlag == 1)
		{ 			  

		if(event_Cancel_flag)
		{

#ifdef DEBUG_LOGS
				  printf("\n\n...Cancelling job due to REQ_CANCEL event...\n\n");
#endif		

		}
		  event_Cancel_flag = FALSE;
		  OIL_JobCancel();
		  OIL_ProbeLog(SW_TRACE_OIL_RASTERCALLBACK, SW_TRACETYPE_EXIT, (intptr_t)1);
		  return TRUE;
		  }
		  if(event_Exit_Paper_flag == TRUE)
		  {
#ifdef DEBUG_LOGS
		  printf("\n\n...Cancelling job due to REQ_Exit_Paper(Form Feed) event...\n\n");
#endif		
		  event_Exit_Paper_flag = FALSE;
		  OIL_JobCancel();
		  OIL_ProbeLog(SW_TRACE_OIL_RASTERCALLBACK, SW_TRACETYPE_EXIT, (intptr_t)1);
		  return TRUE;
		  }
...
void DummyPageforDuplex(void *pJobContext)
{
  int GPSFramePrintretval,GPSFrameCreateExretval,GPSFrameCreateInfoResretval,GPSFrameCloseretval;
  int *framecreate_result;
  gwmsg_GpsPage_FrameCreate_Res_t	pFCR;
  /*check for duplex */
  if(l_PageDuplex)
  {
    /* check page count */
    if((g_pstCurrentJob->uPagesToPrint & 0x1) && (PageCount & 0x1))/* PageCount will increment after every page printing*/
    {
	...
	printf ( " GPS_FrameClose:%s",(!GPSFrameCloseretval) ? "success":"Failure");
#endif     
        if(!g_pstCurrentPage)
        return;
 ...
 /* set up raster data for callback functions to be called */
  ...
  for(j= 0; j < ptRasterDescription->nSeparations; j++)
  {
    ...
    for(i= 0; i < nBands; i++)
    {
      ...
      OIL_RasterDestination(pJobContext, &RasterDestination, ptRasterDescription);
      /* calculate band size */
        ...
        if(Map[k] != OIL_InvalidColor)
        {
          if (g_ConfigurableFeatures.eBandDeliveryType == OIL_PUSH_BAND)
          {
            if(g_pstCurrentPage->eColorantFamily == OIL_ColorantFamily_RGB)
              memset(ptBandPacket->atColoredBand[k].pBandRaster, 255, nBandSize);
            else
              memset(ptBandPacket->atColoredBand[k].pBandRaster, 0, nBandSize);
            ptBandPacket->atColoredBand[k].cbBandSize = nBandSize;
            ptBandPacket->atColoredBand[k].uBandHeight = nBandHeight;
          }
          else
          {
            if(g_pstCurrentPage->eColorantFamily == OIL_ColorantFamily_RGB)
