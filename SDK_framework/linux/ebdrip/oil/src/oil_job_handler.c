...
static int CreateOILJobNode(OIL_TyJob *pstJob);
...
int OIL_SetBinding(OIL_TyJob *pstJob);
...
#ifdef DEFAULT_PDL
  /* Check for the valid mode to initiate the TEC calculations  */
  if((pstJob->eColorMode == OIL_Mono) && (pstJob->uXResolution == 600) && ((g_SystemState.nOILconfig & 0x100) != 0))
    OIL_SetTECData(&g_pPMSCriteria[0]);
  else
    OIL_SetTECData(NULL);
#endif
...
//  pstJob->bOutputDepthMatchesRIP = pms_ptJob->bOutputBPPMatchesRIP;
  ...
  OIL_SetBinding(pstJob);	/*To update bTumble*/
  ret = CreateOILJobNode(pstJob);
 if(!ret)
 	pstJob = NULL;

#ifdef DEFAULT_PDL
  /* Check for the valid mode to initiate the TEC calculations  */
  if((pstJob->uXResolution == RESOLUTION_600))
    OIL_SetTECData(&g_pPMSCriteria[0]);
  else
    OIL_SetTECData(NULL);
#endif
...
static int CreateOILJobNode(OIL_TyJob *pstJob)
{
  ...
  {
 	return 0; // Return NULL if memory allocation failed
  }
  return 1;
}
...
/*	Function Name : 	OIL_SetBinding()
	Input Parameter:	Pointer to Oil job structure, OIL_TyJob *pstJob	
	Functionality : 	OIL_SetBinding() set Tumble value for a duplex job on the basis of 
						Orientaion and Binding.*/
int OIL_SetBinding(OIL_TyJob *pstJob)
{
 int32 result = 0;
 if((pstJob->uOrientation == 0 )&& (pstJob->bDuplex == 1))
 {
	  switch(pstJob->uBindingType)
	 {
		 case GPS_PENV_VAL_INT_BINDING_LEFT:
			pstJob->bTumble = 0;
		 break;
		 case GPS_PENV_VAL_INT_BINDING_RIGHT:
			pstJob->bTumble = 0;
		 break;
		 case GPS_PENV_VAL_INT_BINDING_TOP:
			pstJob->bTumble = 1;
		 break;
		 case GPS_PENV_VAL_INT_BINDING_LONGEDGE:
			pstJob->bTumble = 0;
		 break;
		 case GPS_PENV_VAL_INT_BINDING_SHORTEDGE:
			pstJob->bTumble = 1;
		 break;
		 default:
			pstJob->bTumble = 0;
 ...
 else if((pstJob->uOrientation == 1) && (pstJob->bDuplex == 1))
 {
	 switch(pstJob->uBindingType)
	 {
		 case GPS_PENV_VAL_INT_BINDING_LEFT:
