...
extern int GPS_PlotClose(gwmsg_client_t *client, int plotid);
extern int GPS_SinClose(gps_hostbuf_p host_inBuff);
extern int GPS_SoutClose(gps_hostbuf_p host_outBuff);
extern void GPS_InterpFinishProcData(gwmsg_client_t *client, int ujobid, int fin, int uel_found);
extern void GPS_InterpNotifyState(gwmsg_client_t *, int);
...
#endif
    if (fSuccess)
      fSuccess = RegisterResourceType( EPMS_HWA1_API,
                                       RDR_CLASS_API, RDR_API_HWA );
    if (fSuccess)
      fSuccess = RegisterResourceType( EPMS_HWA1_GIPA2_CMM,
                                       RDR_CLASS_HWA, RDR_HWA_GIPA2_CMM );
    if (fSuccess)
      fSuccess = RegisterResourceType( EPMS_HWA1_GIPA2_BLACK,
                                       RDR_CLASS_HWA, RDR_HWA_GIPA2_BLACK );
    if (fSuccess)
      fSuccess = RegisterResourceType( EPMS_HWA1_GIPA2_GAMMA,
                                       RDR_CLASS_HWA, RDR_HWA_GIPA2_GAMMA );
    if (fSuccess)
      fSuccess = RegisterResourceType( EPMS_HWA1_GIPA2_USER,
                                       RDR_CLASS_HWA, RDR_HWA_GIPA2_USER );
    if (fSuccess)
      fSuccess = RegisterResourceType( EPMS_HWA1_GIPA2_DITHER,
                                       RDR_CLASS_HWA, RDR_HWA_GIPA2_DITHER );
    if (fSuccess)
      fSuccess = RegisterResourceType( EPMS_HWA1_GIPA2_TONERLIMIT,
                                       RDR_CLASS_HWA, RDR_HWA_GIPA2_TONERLIMIT );
    if (fSuccess)
      fSuccess = RegisterResourceType( EPMS_PCL5RESOURCE_API,
                                       RDR_CLASS_API, RDR_API_PCL5RESOURCE );
#ifdef DEFAULT_PDL
    if (fSuccess)
      fSuccess = RegisterResourceType( EPMS_TEC_API,
                                       RDR_CLASS_API, RDR_API_TEC );
#endif
    return fSuccess;
}
