...
void GetTrayInformation(char *pBuf)
{
...
#endif
  {
    ...
    if((g_pstTrayInfo[i].nPriority >= 0) && (g_pstTrayInfo[i].nPriority <= g_nInputTrays))
		InputTrayPriority[i] = g_pstTrayInfo[i].nPriority;
...
static int GetMediaLeadingEdge(PMS_ePaperSize ePaperSize)
{
  switch(ePaperSize)
  {
#if 0 /* commented as mapping done for GPS macros */
    ...
    case PMS_SIZE_DONT_KNOW:
      return 0;
#endif
    case PMS_SIZE_8Hx11T:
    case PMS_SIZE_A4T:
    case PMS_SIZE_8Hx14T:
    case PMS_SIZE_7Qx10HT:
    case PMS_SIZE_A3T:
    case PMS_SIZE_11x17T:
    case PMS_SIZE_A5T:
    case PMS_SIZE_A6T:
    case PMS_SIZE_C5T:
    case PMS_SIZE_DL_T:
