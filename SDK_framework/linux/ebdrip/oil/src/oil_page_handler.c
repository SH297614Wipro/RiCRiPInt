...
void nowDatetime(char*, char*, int);
...
#ifdef DEFAULT_PDL
  ptPage->uTECValue = -1; //Initialize to -1, as the correct values <0,1,2> will be updated at end of page
#endif
  return ptPage;
}
...
#else
        ptBandPacket->atColoredBand[j].ePlaneColorant = (OIL_eColorant)(Map[j] + nColorFamilyOffset);
#endif
      }
    }
  }
...
PMS_TyJob *pms_ptJob = NULL;
  ...
  if(g_pstCurrentJob->bTestPagesComplete == FALSE)
  {
    ...
    if(pms_ptJob->tDefaultJobMedia.ePaperSize  == PMS_SIZE_LETTER)
      pPaper = SetLetter;
#else
    if(g_tJob.tCurrentJobMedia.ePaperSize  == PMS_SIZE_8Hx11)
      pPaper = SetLetter;
#endif
...
/**
 * \brief Create a PCL Banner page 
 *
 * This function creates a series of pcl command to generate a pcl Banner details
 *.The function is called once for each job required
 *.and then to complete the read cycle when it should return an empty string 
 * \param[out]   sz         A pointer to the buffer for storing the string.
 */
void CreatePCLBannerPage(unsigned char *sz)
{
#define DATETIME_BUFFERSIZE 30
char UserName[GPS_MAX_UPARAM_AUTHUSERNAME_LEN];
char JobName[GPS_MAX_UPARAM_JOBNAME_LEN];
char HostName[GPS_MAX_UPARAM_JOBNAME_LEN];
char sDate[DATETIME_BUFFERSIZE];
char sTime[DATETIME_BUFFERSIZE];
enum {EXECUTIVE = 0, LETTER, LEGAL, A4, A3, MONARCH, COMMERCIAL_10, INTERNATIONAL_DL, INTERNATIONAL_C5, CUSTOM_PAPER};
char pagesetup[10][8] = {"\x1b&l1A", "\x1b&l2A", "\x1b&l3A", "\x1b&l26A", "\x1b&l27A", "\x1b&l80A", "\x1b&l81A", "\x1b&l90A", "\x1b&l91A", "\x1b&l101A"};
unsigned char * p = sz;
unsigned int pagesize = 0;
gps_ujobparam_var_t ujobparam_var;

  if (g_pstCurrentJob->bBannerPagesComplete)
  {
    sz[0] = '\0';
    return;
  }
  ...
  //Get Host Name
  memset(&ujobparam_var, 0,sizeof(ujobparam_var));
  ujobparam_var.flag = GPS_UPARAM_HOSTNAME;
  gpsUjobGetParamVar(gps_client, gpsJobId, &ujobparam_var);
  strcpy(HostName,(char*)ujobparam_var.up.hostname);
  //Get Job Name
  memset(&ujobparam_var, 0,sizeof(ujobparam_var));
  ujobparam_var.flag = GPS_UPARAM_JOBNAME;
  gpsUjobGetParamVar(gps_client, gpsJobId, &ujobparam_var);
  strcpy(JobName,(char*)ujobparam_var.up.jobname);
  //Get date & time.  
  nowDatetime(sDate, sTime, DATETIME_BUFFERSIZE);

  switch(g_pstCurrentJob->tCurrentJobMedia.ePaperSize)
  {
  ...
  p += sprintf((char *)p, "\33&a8R\33&a0H\33&l8D\33(8U\33(s0p3h0s3b4099T%s ", UserName);
  p += sprintf((char *)p, "\33&a16R\33&a0H\33&l8D\33(8U\33(s0p8h0s0b4099TJob : %s ", JobName);
  p += sprintf((char *)p, "\33&a19R\33&a0H\33&l8D\33(8U\33(s0p8h0s0b4099THost : %s ", HostName);
  p += sprintf((char *)p, "\33&a22R\33&a0H\33&l8D\33(8U\33(s0p8h0s0b4099TDate : %s ", sDate);
  p += sprintf((char *)p, "\33&a25R\33&a0H\33&l8D\33(8U\33(s0p8h0s0b4099TTime : %s ", sTime);
  
  /* add a form feed */
  p += sprintf((char *)p, "\14");

  g_pstCurrentJob->bBannerPagesComplete = TRUE;
  
}

/**
 * \brief Get Job User ID 
 *
 * This function feaches Job User ID from gps.
 * \param[out]   sz         A pointer to the buffer for storing the string.
 */
void getUserName(char* sName)
{
  gps_ujobparam_var_t ujobparam_var;
  ...
  memset(&ujobparam_var, 0,sizeof(ujobparam_var));
  ujobparam_var.flag = GPS_UPARAM_AUTHUSERNAME;
  if( 0 == gpsUjobGetParamVar(gps_client, gpsJobId, &ujobparam_var) )
  {
    strcpy(sName,(char*)ujobparam_var.up.authusername);
  }
  if(strlen(sName) == 0)
  {
    memset(&ujobparam_var, 0,sizeof(ujobparam_var));
    ujobparam_var.flag = GPS_UPARAM_AUTHUSERNAMECHARSET;
    if( 0 == gpsUjobGetParamVar(gps_client, gpsJobId, &ujobparam_var) )
  	//strcpy(UserName,(char*)ujobparam_var.up.authusernamecharset);
  	;;
  }
  if(strlen(sName) == 0)
  {
    memset(&ujobparam_var, 0,sizeof(ujobparam_var));
    ujobparam_var.flag = GPS_UPARAM_USERID;
    if(0 == gpsUjobGetParamVar(gps_client, gpsJobId, &ujobparam_var) )
  	  strcpy(sName,(char*)ujobparam_var.up.userid);
  }
  if((strlen(sName) == 0) || (sName[0] == '?'))
  {
    memset(&ujobparam_var, 0,sizeof(ujobparam_var));
    ujobparam_var.flag = GPS_UPARAM_HOSTLOGINNAME;
    if(0 == gpsUjobGetParamVar(gps_client, gpsJobId, &ujobparam_var) )
  	  strcpy(sName,(char*)ujobparam_var.up.hostloginname);
  }
 ...
 *
 * This function feaches system Date & Time.
 * \param[out]   sdate         A pointer to the buffer for storing the string.
 * \param[out]   stime         A pointer to the buffer for storing the string.
 */
void nowDatetime(char *sdate, char *stime, int iBuffersize)
{
struct timeval tv;
time_t curtime;
  ...
  curtime=tv.tv_sec;
  strftime(sdate,iBuffersize,"%F",localtime(&curtime));
  strftime(stime,iBuffersize,"%R",localtime(&curtime));
}
