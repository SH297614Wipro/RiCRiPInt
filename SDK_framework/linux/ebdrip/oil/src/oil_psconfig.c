...
static char pszConfigXPS1[] =
      ...
      "openZIP         \n"
      "doXPS           \n";
/*Configuration array for XPS N-up*/
static char pszConfigXPS1_1[] =
      "%!PS                                       \n"
      "/unmountZIP {                              \n"
      "   mark (%XPSStream%) devstatus            \n"
      "   {cleartomark (%XPSStream%) devdismount} \n"
...
/*Configuration array for PDF N-up*/
static char pszConfigPDF1_1[] =
      "                       \n"
      "(%%pdfspool%%) (r) file << /OwnerPasswords [ (%s) ] >> pdfexec \n";
static char pszConfigPDF2[] =
      "(%%embedded%%/PDFdirect) (r) file << /OwnerPasswords [ (%s) ] >> pdfexec \n";
/*Configuration array for PDF N-up*/
static char pszConfigPDF2_1[] =
      "                       \n"
      "(%%embedded%%/PDFdirect) (r) file << /OwnerPasswords [ (%s) ] >> pdfexec \n";
...
BOOL SetupPDLType(BOOL bSendFlag, char *pBuff, unsigned int nBuffSize)
{
  ...
  case OIL_PDL_XPS:
    if(g_pstCurrentJob->bFileInput)
    {
      if(nup > 1)
      {
        sprintf(nuparray,"%d %d %d %s", Nup_Across, Nup_Down, Nup_Layout, "GG_ConfigureNup");
        memcpy(&pszConfigXPS2_1[332], nuparray, strlen(nuparray));
        pszConfigPDL = pszConfigXPS2_1;
      }
      else	
    ...
    else
    {
      ...
      {
        sprintf(nuparray,"%d %d %d %s", Nup_Across, Nup_Down, Nup_Layout, "GG_ConfigureNup");
        memcpy(&pszConfigXPS1_1[968], nuparray, strlen(nuparray));
        pszConfigPDL = pszConfigXPS1_1;
      }
      else	
  ...
  case OIL_PDL_PDF:
    ...
    if(pszConfigPDL)
    {
      ...
      if(g_pstCurrentJob->bFileInput)
      {
        ...
        {
          sprintf(nuparray,"%d %d %d %s", Nup_Across, Nup_Down, Nup_Layout, "GG_ConfigureNup");
          memcpy(&pszConfigPDF2_1, nuparray, strlen(nuparray));	
          sprintf(pszConfigPDL, pszConfigPDF2_1, g_pstCurrentJob->szPDFPassword); /*uses embedded dev to print direct */
        }
        else	
      ...
      else
      {
        ...
        {
          sprintf(nuparray,"%d %d %d %s", Nup_Across, Nup_Down, Nup_Layout, "GG_ConfigureNup");
          memcpy(&pszConfigPDF1_1, nuparray, strlen(nuparray));	
          sprintf(pszConfigPDL, pszConfigPDF1_1, g_pstCurrentJob->szPDFPassword); /* uses pdfspool to spool pdf*/
        }
        else	
...
int SetupPCLMedia( char *pBuff )
{
  ...
  for (i =0; i < NUMFIXEDMEDIATYPES; i++)
  {
	...
	if(PMS_GetMediaType(i, &ThisMediaType))
{
