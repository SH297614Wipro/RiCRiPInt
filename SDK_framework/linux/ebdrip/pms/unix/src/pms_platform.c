...
/**********************************************
 * Directory test and create functions.
 **********************************************/

/* Test for a directory already existing */
int PMS_Directory_Exists(char *szDir)
{
  struct stat sb;

  return (stat(szDir, &sb) == 0 &&
          (S_ISDIR(sb.st_mode)));
}

/* Make a single directory if it does not exist */
static int PMS_Directory_Make(char *szDir)
{
  return (PMS_Directory_Exists(szDir) ||
          mkdir(szDir, 0755) == 0);
}
...
int PMS_Directory_Create(char *szDir)
{
  char path[PATH_MAX];
  size_t dir_len;
  char *new_path;
  ...
  /* Check for useful directory name length */
  dir_len = strlen(szDir);
  if (dir_len == 0 || dir_len >= PATH_MAX) {
    return (0);
  }

  /* Must start with root directory */
  if (szDir[0] != OS_SEPARATOR) {
    return (0);
  }

  /* Walk directory path creating new directories where missing */
  new_path = path;
  separator = 0;
  for (*new_path++ = *szDir++; *szDir != NUL; szDir++) {
    separator = (*szDir == OS_SEPARATOR);
    if (separator) {
      *new_path = NUL;
      if (PMS_Directory_Make(path) == 0) {
        return (0);
      }
    }
    *new_path++ = *szDir;
  ...
  *new_path = NUL;
  return (PMS_Directory_Make(path));
}

/* Join directory paths szPath1 and szPath2 with the Windows directory path separator.
 * szDir must point to a buffer large enough for the joined directory path
 * including the addition separator and terminating NUL.
 */
int PMS_Directory_Join(char *szDir, size_t size, char *szPath1, char *szPath2)
{
  if (size == 0) {
    return (0);
  }

  if (szPath1 == NULL || *szPath1 == NUL ||
      szPath2 == NULL || *szPath2 == NUL) {
    return (0);
  }

  /* Loop copies terminating NUL so need to backtrack when it exits */
  while ((*szDir++ = *szPath1++) != NUL) {
    if (--size == 0) {
      return (0);
    }
  }
  ...
  /* Only add separator if there isn't one there already */
  if (*szDir != OS_SEPARATOR) {
    *szDir++ = OS_SEPARATOR;
    if (--size == 0) {
      return (0);
    }
  ...
  /* Loop copies terminating NUL */
  while ((*szDir++ = *szPath2++) != NUL) {
    if (--size == 0) {
      return (0);
    }
  }
